...
const App = () => {
  ...
  const handleChangeCompleted = (id, checked) => {
    ...
    axios.put(`${url}/${id}`, {completed: !checked})
      .then(() => {
  ...
  const handleSaveEditedTodo = (editedText) => {
    const updatedTodos = todos.map((todo) =>
      todo.id === editingTodoId ? { ...todo, title: editedText } : todo
    );
    axios.put(`${url}/${editingTodoId}`, { title: editedText })
      ...
      <div className='container'>
          <div className='add-do'>
            <input
              ...
              className='add-do__input' 
              type="text" 
              onChange={(e) => setTodoTitle(e.target.value)}
              value={todoTitle}
            />
            <button onClick={handleAddTodo} className='add-do__btn'>Add</button>
          </div>
          <EditModal
            ...
            initialText={editingTodoId ? todos.find((todo) => todo.id === editingTodoId).title : ''}
          />
          <div className='tasks'>
          {
            todos.map(todo => 
                <div key={todo.id} className='task-item'>
                  <div className='task-title'>
                    <input type="checkbox" checked={todo.completed} onChange={() => handleChangeCompleted(todo.id, todo.completed)} />
                    <p className='task-title'>{todo.title}</p>
                  </div>
                  <span>{dayjs(todo.data).format('DD:MM:YYYY HH:mm')}</span>
                  <div className='action-buttons'>
                    <button className='task-btn' onClick={() => handleEditClick(todo.id)}>
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#808080" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 14.66V20a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h5.34"></path><polygon points="18 2 22 6 12 16 8 16 8 12 18 2"></polygon></svg>
                    </button>
                    <button className='task-btn' onClick={() => handleDeleteTodo(todo.id)}>
